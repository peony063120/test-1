...
const Login = () => {
  ...
  const onSubmit = async (values: LoginPayload) => {
    try {
      ...
      const target = redirectTo && redirectTo !== '/' ? redirectTo : getDefaultRouteForUser(response.user);
    ...
    <Container maxWidth="xs">
      <Paper sx={{ p: 4 }}>
        ...
        <Box component="form" noValidate onSubmit={handleSubmit(onSubmit)}>
          <TextField {...register('username')} label="Tên đăng nhập hoặc email" fullWidth margin="normal" autoComplete="username" error={Boolean(errors.username)} helperText={errors.username?.message} />
          ...
          <Button component={Link} to="/register" fullWidth sx={{ mt: 1 }}>Chưa có tài khoản? Đăng ký</Button>
        </Box>
      </Paper>
    </Container>
  </Box>;
